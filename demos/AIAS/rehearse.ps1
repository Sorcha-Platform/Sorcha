...
#     register, presented into a workflow on another).
#
...
function Get-CyberPresentation {
    # Export the applicant's own AssuredIdentityCredential as SD-JWT (mirrors the proven
    # walkthroughs/TradeFinance/run.ps1 present-a-held-credential pattern) and shape it into the
    # CredentialPresentation the cyber blueprint's action-1 credentialRequirements gate expects
    # (Sorcha.Blueprint.Models.Credentials.CredentialPresentation: credentialId/disclosedClaims/
    # rawPresentation). Submitting this directly on the action body takes the SYNCHRONOUS internal
    # verifier branch in ActionExecutionService rather than initiating the async F111 HAIP/SorchaWallet
    # presentation lifecycle — the same branch TradeFinance's -CredentialPresentations exercises.
    param([Parameter(Mandatory)][object]$Applicant, [Parameter(Mandatory)][object]$Credential)
    ...
    $disclosed = Get-SdJwtDisclosedClaims -SdJwt $rawToken
    return @(
        @{
...
function Submit-CyberQuestionnaire {
    param(
        ...
        [Parameter(Mandatory)][array]$CredentialPresentations
    )
    $instance = Invoke-SorchaApi -Method POST `
        -Uri "$api/instances/" `
        -Body @{
    ...
    $null = Invoke-SorchaAction `
        ...
        -PayloadData $Answers -CredentialPresentations $CredentialPresentations -WaitForSeal
    return $instanceId
}
...
function Test-CyberCredentialDelivered {
    ...
    while ((Get-Date) -lt $deadline) {
        try {
            ...
            if ($snap.credentials -and $snap.credentials.Count -gt 0) {
                $hit = $snap.credentials | Where-Object {
                    $_.vct -match "cyber-level" -or $_.vct -match "CyberLevel" -or
                    $_.credentialType -match "CyberLevel" -or $_.displayLabel -match "Cyber"
                } | Select-Object -First 1
                if ($hit) {
...
try {
    ...
    $presentation = Get-CyberPresentation -Applicant $mint.Applicant -Credential $mint.Credential
    $instId = Submit-CyberQuestionnaire -Applicant $mint.Applicant -Answers $script:CyberAnswersPerfect -CredentialPresentations $presentation
...
try {
    ...
    $presentation = Get-CyberPresentation -Applicant $mint.Applicant -Credential $mint.Credential
    $instId = Submit-CyberQuestionnaire -Applicant $mint.Applicant -Answers $script:CyberAnswersTrapped -CredentialPresentations $presentation
    Write-WtInfo "submitted cyber questionnaire (instance=$instId)"
...
try {
    ...
    $presentation = Get-CyberPresentation -Applicant $mint.Applicant -Credential $mint.Credential
    $instId = Submit-CyberQuestionnaire -Applicant $mint.Applicant -Answers $script:CyberAnswersDire -CredentialPresentations $presentation
    Write-WtInfo "submitted cyber questionnaire (instance=$instId)"
