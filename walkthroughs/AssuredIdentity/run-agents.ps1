...
# ---------- Env vars ----------
# Passwords are handed off to the spawned agent processes via process-scope
# env vars (the actor JSON files reference them as `$env:NAME`). Process-scope
# means they don't leak to the user's shell session and are only visible to
# child processes started below. Source: state.json (gitignored, generated by
# setup.ps1). Cleanup at end of script is unconditional, even on error paths.
# This is a dev/demo tradeoff — production agents would use a secret manager.
...
foreach ($actor in $actors) {
    ...
    $errLogFile = Join-Path $logsDir "$($actor.Name).err.log"
