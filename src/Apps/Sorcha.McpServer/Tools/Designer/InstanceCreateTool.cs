...
public sealed class InstanceCreateTool
{
    ...
    [Description("Starts a running workflow instance from a blueprint that has already been published to a register, returning the instanceId needed for every subsequent step. Call this when sorcha_blueprint_publish has already put the blueprint live on the target register and you need to begin a fresh execution of it: pass the returned instanceId to sorcha_action_submit to perform the workflow's first action, then poll sorcha_workflow_status to observe the outcome, since submission in Sorcha is asynchronous and neither call reports workflow progress directly. Both blueprintId and registerId are required, and the blueprint must already be published to that register by sorcha_blueprint_publish — publishing and instantiating are separate steps. The returned instanceReference is a human-readable label auto-generated from the first action's payload, so it is typically null immediately after creation and only appears once that first action has been submitted and folded.")]
