...
1. `Seed:ServicePrincipals:{clientId}` (env `Seed__ServicePrincipals__{clientId}`) — the per-deploy
   ...
   agree. Wins in every environment.
2. **Development** with no configured secret falls back to the committed dev literal (e.g.
   `blueprint-service-secret`) — keeps a bare `docker compose up` working for local dev.
3. **Production/Staging** with neither of the above **fails closed** (`InvalidOperationException` at
...
4. Any other environment (e.g. Testing) with neither of the above generates a fresh random secret.

The resolved source (`Configured` / `DevLiteral` / `Generated`) is logged; the secret value itself
never is.
