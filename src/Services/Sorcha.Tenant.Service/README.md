...
- **Assurance model** — `AssurancePolicy` is the server-authoritative, computed-never-stored source of truth: `AuthAssuranceTier` (`Basic < Strong < Strongest`), `TierOfMethod`, `TierOfProof`, `RequiredProofTier(operation, target)`. **Password-as-proof is Strong by a flagged decision (T061).**
