...
public sealed class TenantMembershipInboxWriter : ITenantMembershipInboxWriter
{
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            {
                _logger.LogWarning(
                    "Inbox skip — {PlatformUserId} does not name a known platform user for org membership added (Org={OrgId})",
        ...
        CancellationToken ct = default)
    {
        try
        {
            // #1703 sweep — see WriteOrgMembershipAddedAsync.
            if (!await _inbox.PlatformUserExistsAsync(platformUserId, ct).ConfigureAwait(false))
            {
                _logger.LogWarning(
                    "Inbox skip — {PlatformUserId} does not name a known platform user for org membership role change (Org={OrgId})",
                    platformUserId, organizationId);
                return;
            }
