...
5. **Primitive location:** a new **pure-managed project `Sorcha.Cryptography.Secp256k1`** (BouncyCastle only) — **no new dependency** (BouncyCastle is already in `Directory.Packages.props`). **Correction (Phase 0, feature 177 `research.md` §R6):** the earlier plan to put this in a *folder inside `Sorcha.Cryptography`* is infeasible — `Sorcha.Verifier.Engine` is consumed by the Blazor **WASM** PWA and must stay native-dependency-free, so it cannot reference `Sorcha.Cryptography` (native `Sodium`/`Mcl`). Both verification paths need secp256k1, and .NET's built-in `ECDsa` doesn't reliably support secp256k1 on Windows/WASM — so the primitive is a shared pure-managed leaf referenced by `Sorcha.Cryptography`, `Sorcha.Verifier.Engine`, `Sorcha.ServiceClients.Http`, and `Sorcha.Blueprint.Service`. It still adds no `WalletNetworks` signing-enum member.
...
Phase 1 spans **five** projects — one new (`Sorcha.Cryptography.Secp256k1`) plus edits to four existing homes that are *reused*, not forked. (See §R6 correction in decision 5: the primitive is a shared pure-managed project, not a folder in `Sorcha.Cryptography`.)

| Project | Change |
|---|---|
| **`src/Common/Sorcha.Cryptography.Secp256k1`** (NEW) | The primitive: ES256K verify + keccak256 + secp256k1 JWK + EIP-55. Pure-managed (BouncyCastle only), WASM-safe. Referenced by the four projects below. |
| **`src/Common/Sorcha.Cryptography`** | **Edit** `SdJwt/SdJwtService.cs` (`Verify` + `MapAlgorithm` + `ExportPublicKeyFromJwk`) to add the ES256K branch (issuer JWS + KB-JWT), delegating to the primitive. Add ProjectReference. |
