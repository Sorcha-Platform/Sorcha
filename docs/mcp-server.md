...
Use this slice when an agent has admin scope and needs to inspect or operate a running instance. Tools include `sorcha_health_check` (aggregated platform health), `sorcha_metrics` (snapshot of platform metrics), `sorcha_audit_query` (your organisation's audit log, including refusals, with the reason each service recorded; this is the tool to use when something was refused, #1648), `sorcha_log_query` (deliberately NotSupported: raw service logs are an operator surface), `sorcha_peer_status` and `sorcha_validator_status` (P2P + consensus visibility), `sorcha_register_stats` (per-register statistics), `sorcha_tenant_create` / `sorcha_tenant_list` / `sorcha_tenant_update` (tenancy management), `sorcha_user_list` / `sorcha_user_manage` / `sorcha_token_revoke` (user and credential management). It also covers register federation (subscribe/unsubscribe/sync-state/relationship) and credential lifecycle (offer/suspend/reinstate/revoke/refresh). This list is illustrative, not exhaustive — it's the largest slice by tool count; see the per-slice table in [`src/Apps/Sorcha.McpServer/README.md`](../src/Apps/Sorcha.McpServer/README.md) for the current breakdown.

`sorcha_register_governors` (#1647) answers "who governs this register?" — every named Owner/Admin/Designer/Auditor on a register's governance roster, reconstructed from its sealed Control transactions. It exists for the moment a governance action or `sorcha_blueprint_publish` is refused with a 403: `sorcha_register_relationship` reports only the caller's (or this node's) own derived role, so it cannot name who else could actually approve the thing that was just refused. `NotFound` means no *sealed* roster yet — an unknown register id, or a brand-new one whose genesis has not sealed — never that the register itself does not exist; a 4xx from the Register Service is reported as a refusal, not an outage, and a 403 does not count against the service's availability (#1673). Carries the same `sorcha:admin` entitlement as `sorcha_register_relationship`, since both sit behind the Register Service's `CanReadTransactions` policy.
...
| URI | Returns |
|---|---|
| `sorcha://schema/blueprint` | The embedded blueprint JSON Schema. Describes both routing mechanisms (the `routes` array and action-level `participants`/`condition` — neither is legacy), `isStartingAction`, the full credential surface (`credentialRequirements`, `credentialIssuanceConfig`, `trustPolicy`, `claimMappings`, etc.), `rejectionConfig`, `requiredPriorActions`, and `instanceReference`. Kept current against the models by a reflection-based CI gate (`BlueprintSchemaCurrencyTests`). Read this before writing any blueprint JSON. |
