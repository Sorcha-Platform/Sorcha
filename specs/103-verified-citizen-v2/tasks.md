...
- [x] T041 [US2] Wire `SchemaRefResolver` into `src/Services/Sorcha.Validator.Service/Services/ValidationEngine.cs` so that the flatten step runs **before** the existing x-* stripping at the validation entry point. Cache the flattened form keyed by the consuming blueprint id and version. Register the resolver in DI.
