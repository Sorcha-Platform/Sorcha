...
- **A schema change does not appear in your database.** Pre-release, schema changes are folded into `InitialCreate` rather than stacked as new migrations — which a database that already recorded `InitialCreate` will never pick up (`42703: column … does not exist`). Recreate the database; do not expect `MigrateAsync` to help.
