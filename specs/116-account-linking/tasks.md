...
- [X] T051 [US2] Modify `PasskeyEndpoints.RegisterVerify` in `src/Services/Sorcha.Tenant.Service/Endpoints/PasskeyEndpoints.cs` — require non-empty `DisplayName` in the request DTO (FluentValidation + 400 on empty) **[Already enforced at `RegisterOptions` time — DisplayName is captured into the registration cache before `RegisterVerify`, so the existing validation gate at options time covers the contractual requirement]**
- [X] T052 [US2] Add `PUT /api/passkey/credentials/{id}` to `PasskeyEndpoints` per `contracts/passkey-management.openapi.yaml` — updates `DisplayName`; rejects with 409 when target passkey has `Status=Disabled`; no challenge required **[Path uses singular `passkey` to match existing route group; contract YAML updated to match]**
- [X] T053 [US2] Modify `PasskeyEndpoints.DeleteCredential` — soft-revoke per data-model.md state-transition table: set `Status=Revoked`, `DisabledAt=now`, `DisabledReason="user-removed"` (or `"user-removed-after-disable"` when starting from Disabled); `[RequireAuthChallenge(RemoveAuthMethod)]` for Active passkeys, bypass for Disabled; floor check via `IAuthMethodService.WouldRemovingLeaveZero` inside same transaction **[Challenge gating implemented inline in the handler rather than via the filter so it can branch on credential status — Disabled bypasses, Active gates. Inline path mirrors the filter's 5-step protocol and emits the same telemetry. Floor check happens inside `IPasskeyService.RevokeCredentialAsync` in the same SaveChanges as the mutation.]**
- [X] T054 [US2] Modify `PasskeyEndpoints.ListCredentials` — exclude `Status=Revoked` by default; the `/api/me/auth-methods` aggregate already does this independently **[Also adopted the `ResolvePlatformUserIdAsync` helper for claim-fallback consistency with the new PUT/DELETE handlers and `AuthMethodsEndpoints`.]**

### Tests

- [X] T055 [P] [US2] Unit tests `tests/Sorcha.Tenant.Service.Tests/Services/PasskeyRevocationTests.cs` — Active → Revoked sets all three columns; Disabled → Revoked uses `"user-removed-after-disable"`; Revoked is terminal; floor blocks last-passkey removal
- [X] T056 [P] [US2] Update integration tests in `tests/Sorcha.Tenant.Service.Tests/Endpoints/PasskeyEndpointTests.cs` — register requires non-empty DisplayName (400), PUT renames an Active passkey (204), PUT against Disabled rejects (409), DELETE Active requires challenge (401 without, 204 with), DELETE Disabled bypasses challenge (204), DELETE last-method blocked (409), Revoked rows excluded from list **[DELETE happy-path consume coverage lives in the SQLite-backed service tests — the WebApplicationFactory's InMemory EF provider does not support `ExecuteUpdateAsync` used by the atomic-consume primitive; this is the same gap documented in `SocialLinkEndpointTests`. Endpoint tests cover anonymous/missing-challenge/Disabled-bypass/floor paths.]**
...
- [X] T063 [P] [US3] Create `IPasswordManagementService` + `PasswordManagementService` at `src/Services/Sorcha.Tenant.Service/Services/PasswordManagementService.cs` — `SetAsync(platformUserId, password)`, `ChangeAsync`, `RemoveAsync`; bootstrap detection (`PasswordHash IS NULL AND no socials AND no active passkeys`); BCrypt hashing via existing `IPasswordHasher`; floor enforcement on remove (depends on T010) **[BCrypt called directly via `BCrypt.Net.BCrypt.HashPassword` to match `PasswordResetService` / `PlatformUserProvisioningService` — there is no `IPasswordHasher` abstraction in the codebase. Bootstrap detection delegates to `IAuthMethodService.GetCountsAsync.Total == 0`.]**
- [X] T064 [US3] Create `PasswordRequest` DTO + `PasswordRequestValidator` (FluentValidation: min length per existing platform policy) at `src/Services/Sorcha.Tenant.Service/Models/Requests/PasswordRequest.cs` **[Validation lives in `IPasswordPolicyService.ValidateAsync` (NIST SP 800-63B) — the existing platform-wide policy — rather than a standalone FluentValidation validator. Outcome translates to a `PolicyViolation` enum value the endpoint maps to 400.]**
- [X] T065 [US3] Add `POST /api/auth/password/set` to `src/Services/Sorcha.Tenant.Service/Endpoints/AuthEndpoints.cs` per `contracts/auth-password.openapi.yaml` — challenge conditional on bootstrap (skip filter when `methods.Count == 0`); 409 if password already set (depends on T063, T064) **[Mounted in a new `PasswordEndpoints.cs` rather than added to `AuthEndpoints.cs` for cohesion — all three password operations live together. Conditional gating done via inline 5-step protocol (filter can't be conditional). Path matches contract.]**
- [X] T066 [US3] Add `POST /api/auth/password/change` to `AuthEndpoints` — `[RequireAuthChallenge(ScopedOperation.ChangePassword)]`; 409 if no current password (depends on T063, T064) **[Same `PasswordEndpoints.cs` mount; uses the filter directly since gating is unconditional.]**
- [X] T067 [US3] Add `POST /api/auth/password/remove` to `AuthEndpoints` — `[RequireAuthChallenge(ScopedOperation.RemovePassword)]`; floor enforcement via `IAuthMethodService.WouldRemovingLeaveZero` inside same transaction (depends on T063)

### Backend — 2FA disable adoption (FR-024)

- [X] T068 [US3] Modify the existing 2FA-disable endpoint in `src/Services/Sorcha.Tenant.Service/Endpoints/TotpEndpoints.cs` — add `[RequireAuthChallenge(ScopedOperation.Disable2Fa)]` to close the hijacked-session unguarded-prune gap (depends on T012) **[Filter enhanced in this PR to fall back from `platform_user_id` claim → `sub` → `IIdentityRepository.GetUserByIdAsync.PlatformUserId` so it works with both production JWTs and test JWTs (TestAuthHandler emits only `sub`). This was a gap surfaced by the new TOTP disable test — addressed inline.]**

### Tests

- [X] T069 [P] [US3] Unit tests `tests/Sorcha.Tenant.Service.Tests/Services/PasswordManagementServiceTests.cs` — set with challenge, set in bootstrap mode (no challenge), change with challenge, remove with challenge + floor protection, BCrypt hash applied (depends on T063)
- [X] T070 [P] [US3] Integration tests `tests/Sorcha.Tenant.Service.Tests/Endpoints/PasswordEndpointTests.cs` — all four endpoints (set / change / remove) with conditional challenge enforcement; bootstrap path; 409 on already-set / no-password / floor; verify password actually rotated (depends on T065-T067) **[Happy-path challenge-consume coverage at the endpoint layer is in the SQLite-backed service tests — same `ExecuteUpdateAsync`-on-InMemory limitation as US1/US2. Endpoint tests cover anonymous / missing-challenge / bootstrap-bypass / second-set-after-bootstrap-rejection paths.]**
- [X] T071 [P] [US3] Integration test update in `tests/Sorcha.Tenant.Service.Tests/Endpoints/TotpEndpointTests.cs` — disable 2FA without challenge token returns 401; with valid `Disable2Fa`-scoped token returns 204 (depends on T068) **[Existing "Disable_AfterSetup_ReturnsDisabledStatus" tests rewritten as `Disable_*_WithoutChallenge_Returns401` — same `ExecuteUpdateAsync` limitation prevents the with-valid-token endpoint test. Service-layer disable coverage gap noted.]**
