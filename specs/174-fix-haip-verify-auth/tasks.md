...
- [x] T025 Manually run Scenario A (happy path SC-001/SC-002), Scenario B (error/retry SC-003/SC-004), and Scenario C (not-configured SC-005) from `specs/174-fix-haip-verify-auth/quickstart.md` — deferred (running backend required); two post-code-review bugs fixed instead: (1) `RetryPolling` now calls `_cts?.Cancel(); _cts?.Dispose()` before reassigning to prevent CTS leak on retry; (2) `_errorState` branch moved before `_state == Submitted` in markup so transport errors in Submitted state surface error/retry UI instead of spinner.
