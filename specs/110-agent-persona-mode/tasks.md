...
- [x] T036 [P] [US3] Integration test in `tests/Sorcha.Agent.Tests/Persona/PersonaReactiveCoexistenceTests.cs` — two tests (persona-first and reactive-first ordering, gated via `TaskCompletionSource`) prove both code paths emit their POSTs against a shared `HttpClient` without blocking each other. Targets the concurrency property directly rather than re-wiring `RunCommand.ExecuteAsync`'s config-loading layer (already covered by `RunCommandTests`).
- [ ] T037 [P] [US3] Benchmark-style test in `tests/Sorcha.Agent.Tests/Persona/PersonaReactiveLatencyTests.cs`: measures reactive-path round-trip with persona absent vs present; asserts ratio ≤ 1.25 (SC-004 threshold); tagged as a perf test so CI can keep it reliable with generous tolerance if flaky — **deferred**: perf tests in the unit suite are a maintenance liability and the ratio guarantee is better verified via the live walkthrough. Tracked as a follow-up.

### Implementation for User Story 3

- [x] T038 [US3] Review `RunCommand.ExecuteAsync` (already modified in T021) and confirm persona task is launched via `_ = personaHost.RunAsync(cts.Token)` BEFORE the inbox `await foreach`, so the persona begins its `delaySeconds` wait while the inbox listener is being constructed; no additional code change anticipated but capture the decision in a short code comment explaining the ordering — comment at `RunCommand.cs:167-170` now cites SC-004 and points at the T039 regression test.
- [x] T039 [US3] Ensure cancellation propagates correctly on `Ctrl+C` / shutdown signal: the `CancellationTokenSource` created at the top of `ExecuteAsync` is shared between inbox loop and persona task; on cancellation both observe it within ≤ 1 s. Add a regression test in `tests/Sorcha.Agent.Tests/Persona/PersonaShutdownTests.cs`
