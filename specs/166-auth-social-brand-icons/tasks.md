...
- [ ] T001 Confirm `MudBlazor 9.5.0` entry in `Directory.Packages.props` and verify `Icons.Custom.Brands.Google/Microsoft/GitHub/Apple` compile in `src/Apps/Sorcha.Wallet.Pwa` — no package change expected per plan.md
...
- [ ] T009 [P] Run `dotnet build src/Services/Sorcha.Tenant.Service` and `dotnet build src/Apps/Sorcha.Wallet.Pwa` — confirm zero new warnings; in particular no missing `/// <summary>` warnings on `SocialProviderBrandIcon` and no nullable warnings introduced by the resolver or view changes
- [ ] T010 [P] Run resolver unit tests and confirm all green: `dotnet test tests/Sorcha.Tenant.Service.Tests --filter "FullyQualifiedName~SocialProviderBrandIcon"`
- [ ] T011 Run the full Tenant Service test suite to confirm no regression: `dotnet test tests/Sorcha.Tenant.Service.Tests`
- [ ] T012 Visual smoke test per quickstart.md §2-3: load web login + signup in browser, toggle light/dark — confirm Google/Microsoft show brand colours; GitHub/Apple marks are legible in both modes (track `currentColor`); then load PWA sign-in and confirm leading icons match passkey button treatment and narrow-mobile viewport shows no overflow (quickstart.md edge case)

**Done when**:
- [ ] All resolver unit tests pass (T010)
- [ ] Web login shows correct, legible, fallback-safe brand icons in light & dark (T005, T012)
- [ ] Web signup shows identical icons to login for the same provider (T006, T012)
- [ ] PWA sign-in shows leading brand icons matching the passkey button treatment (T007, T008, T012)
- [ ] Zero new build warnings introduced (T009)
- [ ] Full Tenant Service test suite green (T011)
