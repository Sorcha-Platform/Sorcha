...
| Password (re-entry) | **Strong** | ⚠️ **Planning decision (flagged for review)** — preserves today's capability to manage the account with the password. The Basic-can't-strip-Strong/Strongest invariant is unaffected. A future hardening pass MAY demote password-as-proof to Basic; that change is isolated to this table. |
...
The required tier equals **the assurance being removed/weakened** ("you can't use a weaker thing to take away a stronger protection").
...
| Remove / Disable | Authenticator (TOTP) | **Strong** | Removes Strong protection. |
| Change | Password | **Strong** | Rotates the knowledge factor. |
| Remove | Password | **Strong** | + last-method floor. |
...
1. A user whose only step-up-capable proof is Email OTP (Basic) **cannot** remove a passkey, disable TOTP, change/remove the password, change the phone, or view/regenerate backup codes. They **can** disable Email/SMS OTP and unlink a social (subject to last-method).
