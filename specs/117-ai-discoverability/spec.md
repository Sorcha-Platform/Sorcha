...
AI agents and AI coding assistants are now a primary consumer of any platform claiming to be "open" or "integrable." Two converging forces make discoverability load-bearing for Sorcha specifically. First, AI-generated fraud is scaling — assertion-based identity and document systems have no defence against high-quality forgery, and Sorcha's cryptographic-proof posture is the answer that AI-aware buyers will look for first. Second, AI systems are becoming decision-makers — by 2028, 80% of governments are projected to deploy AI agents for routine decisions, and the EU AI Act mandates documented provenance for high-risk automated decisions. AI systems making consequential decisions need verified data inputs; Sorcha is the verified-data layer those systems can consume with confidence. Both forces select strongly for platforms that an AI agent can find, parse, and reason about without human mediation.

When such an agent encounters Sorcha today it finds a good `README.md` and then hits a wall: no machine-readable OpenAPI specification served at a stable URL, no MCP manifest under `/.well-known/`, no `llms.txt`, no `STANDARDS.md`, and a quickstart that is human-paced rather than agent-runnable.

The platform already has the substance — a 36-tool MCP server, real OpenID4VCI/OpenID4VP issuance and verification (specs 097/098), HAIP 1.0 conformance work (spec 094 onwards), ML-DSA (NIST FIPS 204) and ML-KEM (FIPS 203) as core not branch-feature, IETF Token Status List 2024 publishing (spec 095), and a clean service decomposition behind a YARP gateway. None of that is presented in a shape an agent can crawl, parse, or reason about without human mediation.
...
**Strategic context input.** All content in machine-readable artefacts (OpenAPI `info.description`, `llms.txt`, MCP tool descriptions, `STANDARDS.md` introduction, the four published `docs/` documents) is authored against the framing in `docs/strategic-context.md`. That document is the canonical source for *what to say* about Sorcha to an AI audience: the problem (assertion-based systems vs. cryptographic proof), the timing (AI fraud + AI decision-makers + EU regulatory pull), the differentiators (vs. Kafka / centralised platforms / public blockchains / anonymity-first P2P), and the honest gaps (HAIP classical-only boundary, SLH-DSA not implemented, BBS+ not implemented). Every authoring task in `tasks.md` references it. Without it, the spec ships shape but not voice.
