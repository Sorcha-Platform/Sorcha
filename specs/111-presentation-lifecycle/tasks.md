...
- [X] T043 [US2] Create `src/Services/Sorcha.Haip.Service/Services/HaipPresentationConsumer.cs` implementing IPresentationConsumer.ConsumerName = "haip" and VerifyAsync mapping HaipVerificationResult → PresentationOutcome (reason-code mapping table per enum)
- [X] T044 [US2] Register `HaipPresentationConsumer` as `IPresentationConsumer` in `src/Services/Sorcha.Haip.Service/Program.cs` DI
