...
- [x] T039 [US2] Wire `IIssuanceKeyService.GetOrDeriveAsync` into the credential-issuance flow. *(IssueCredential endpoint now invokes GetOrDeriveAsync before signing when the request supplies a TenantId — honors FR-004 'no later than first issuance' by lazily deriving the org key + publishing the DID document. The sign step itself still uses the wallet's own key; the kid-swap to `did:sorcha:org:{addr}#vc-issuance-{rotationIndex}` requires SD-JWT-layer kid header changes and is a separate follow-up.)*
