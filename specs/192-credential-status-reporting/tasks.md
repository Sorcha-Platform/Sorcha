...
Gate answers as built:

| Gate | Answer | What it meant in the code |
|---|---|---|
| **A1** | SUSPENDED always refuses | no policy surface added; `CheckStatusAsync` refuses on it unconditionally |
| **B2** | the distinction reaches the holder | `PresentationDeclineReason.Suspended`, which lands on the citizen's own application record |
| **C1** | no suspension analogue for `RevocationCheckPolicy` | that policy still governs only *unresolvable* status; nothing renamed |
| **D2** | new type, `StatusListBit` retired | `CredentialStatusValue { Valid, Invalid, Suspended, Unresolved }` — and it earned its keep: the type change broke **12 call sites**, every one of which had to be looked at |
