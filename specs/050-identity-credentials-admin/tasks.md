...
- [ ] T030 [US4] Add QR code JS interop for presentation requests — create `src/Apps/Sorcha.Admin/wwwroot/js/qrcode-interop.js` with generateQrCode(elementId, data) function using lightweight QR library, and create `src/Apps/Sorcha.Admin/Sorcha.Admin.Client/Interop/QrCodeInterop.cs` wrapper class calling IJSRuntime.InvokeVoidAsync
...
- [ ] T033 [US5] Implement StatusListService in `src/Apps/Sorcha.UI/Sorcha.UI.Core/Services/Credentials/StatusListService.cs` — GetStatusListAsync: GET `/api/v1/credentials/status-lists/{listId}` (public, no auth), returns StatusListViewModel. Handle 404 → null.
