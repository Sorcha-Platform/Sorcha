...
- [X] T033 [P] [US2] Test: approval from an organisation not on the proposal's roster snapshot is ignored, not counted — `GovernanceApprovalTallyTests.AnApprovalFromAnOrganisationThatJoinedAfterTheProposalWasRaised_DoesNotCount`. The pre-existing not-on-roster test could not make this claim: its stranger is on *neither* roster, so it passes identically whichever roster `Prepare` is handed. The new one uses an organisation on the register's roster **today** that was absent from the snapshot, and carries the counterfactual — the same approval counting against the roster it belongs to — so the exclusion is attributable to the snapshot rather than to the approval.
- [X] T034 [P] [US2] Test: a repeat approval by the same organisation leaves the count unchanged — **already covered, nothing added.** `ASecondApprovalFromTheSameOrganisation_IsExcludedAsADuplicate` (count unchanged) and `WhenAnOrganisationVotesTwice_TheFirstStands_AndTheSecondIsExcluded` (first-vote-wins, pinned so it cannot drift to last-wins) both make the claim. A duplicate never reaches `ToVotes`, so it cannot reach the quorum arithmetic.
- [X] T035 [P] [US2] Test: roster change with a proposal open → `Invalidated`, reason `roster-changed`, not enactable thereafter — **already covered across the two layers.** Status: `GovernanceProposalStatusTests.AProposalWhoseRosterMovedUnderIt_IsInvalidated`. Not-enactable: `GovernanceEnactmentServiceTests.AProposalInvalidatedByARosterChange_IsNotEnacted` (the raise side) and `RightsEnforcementServiceTests.ProposalRaisedAgainstAnOlderRoster_IsInvalidated` (`VAL_PERM_009`, the authoritative gate — every node must reach this verdict on a sealed enactment, not just the node that took the submission).
- [X] T036 [P] [US2] Test **SC-010**: under `Unanimous` with one approver outstanding, removing that approver **invalidates** rather than enacts — the security property of the whole feature. `RightsEnforcementServiceTests.RemovingTheLastOutstandingApprover_InvalidatesRatherThanEnacts` existed and asserted the guard fired (`VAL_PERM_009`, plus `ValidateQuorumAsync` never reached). **It claimed the attack in prose and executed none of it** — so it would have passed just as well had the attack never been available, which is precisely a green tick asserting a known attack is defended when nothing checked. Extended with the counterfactual run as arithmetic on the same two rosters: unanimity over the before-roster needs 2 against 1 collected approval; over the after-roster it falls to `<= collected`, i.e. a re-count would enact the change the removed organisation was blocking.
- [X] T037 [P] [US2] Test: `Transfer` never takes the Owner override, even for a sole owner (FR-010) — `GovernanceRosterServiceTests.ValidateQuorumAsync_TransferBySoleOwner_IsNotWavedThroughByTheOverride` + `..._AddBySoleOwner_DoesTakeTheOverride`. The pre-existing `ValidateQuorumAsync_TransferByOwner_NoOwnerOverride` uses a two-member roster, where a zero-approval Transfer fails on ordinary threshold anyway — so it cannot separate "the override was withheld" from "there were not enough votes". The sole-owner roster is the only shape where the override is the *sole* thing between zero approvals and a register changing hands; the paired `Add` proves the refusal is the operation-type carve-out and not a property of one-member rosters.
- [X] T038 [P] [US2] Test: an expired proposal is not enactable by a late approval (FR-012) — `RightsEnforcementServiceTests.AProposalPastItsWindow_IsRefusedByTheValidator` + `..._AProposalInsideItsWindow_IsNotRefusedForExpiry`. Tested at the **Validator**, which is where an expired proposal actually dies: the approve endpoint's 409 stops a late approval being *raised* on that node, but `GovernanceEnactmentService.TryEnactAsync` deliberately checks no window of its own, and every node must reach the same verdict on a sealed enactment. `ValidateProposal` is routed through the **real** rule via `UseTheRealProposalRules()` rather than a stubbed `Failure(...)` — a canned verdict would assert only that the test can return a string it wrote itself, and would survive the rule being deleted.

> **Verification for T033-T038 (mutation-tested, not just green).** A guard written after the code has never run red, so each claim was checked by breaking the source deliberately and confirming the *named* test failed: removing the `Transfer` carve-out reds T037 while its `Add` counterfactual stays green; deleting the expiry check reds T038-expired while T038-inside stays green; disabling the `VAL_PERM_009` block reds SC-010; flooring `Unanimous` at 2 reds **only** SC-010's counterfactual assertion (proving that block executes rather than decorating); demoting `Admin` to non-voting reds T033's counterfactual specifically. All mutations reverted — `git status src/` clean. Full suites: Models 373, Register.Core 329, Validator.Service 1079 passed, 0 failed.
>
> **One limit stated honestly.** Within `GovernanceApprovalTally.Prepare` the snapshot-vs-current distinction is not isolable by mutation, because `Prepare` receives exactly one roster — T033 pins that the parameter is *honoured*, and which roster reaches it is pinned separately by the FR-011b guard tests (T035/T036). That is also why the call site in `GovernanceEnactmentService` passing `GetCurrentRosterAsync(...)` is correct rather than a defect: the FR-011b check earlier in `TryEnactAsync` makes current and snapshot provably identical before the tally runs.
...
- [~] T062 [US4] 🔴 **LIVE GATE** Transfer system register ownership through the governance process; confirm the record replicates to tiny — **RUN 2026-08-17 on n1. The MECHANISM PASSES; the SYSTEM REGISTER is BLOCKED.** Verified at the docket on ordinary register `c3717b023df34eddba39f6c4f1f3d971`: genesis → Transfer proposal *Pending* (202, carrying no roster) → two sealed approvals → **enactment raised by nothing but the `docket:confirmed` reaction** → ownership moved. Replication confirmed on the system register's own control chain (tiny holds n1's SSR docket 5, same tx id). **The SSR itself is unreachable by this path for two independent reasons, both demonstrated rather than argued: #1464** — `ValidateTransferProposal` requires the target to be an existing Admin, the only post-genesis route in is `Add`, and both `Add` sites write `PublicKey = string.Empty` with nothing ever filling it in; roster authority matches *by key*, so promoting a governance-added member leaves a register nobody can govern (proven on `e89a93f7b803…`: the promoted Owner's next proposal returned **200 and never sealed**, `VAL_PERM_002`) — doing this to the SSR would have permanently bricked the platform's root of trust. **#1465** — the SSR's one-member roster is the ceremony Owner, whose key is the node's `validator:*` system wallet; `Transfer` is excluded from the Owner override so it needs that approval, but approvals are detached/external by design (R-014) and `/api/v1/wallets/{address}/sign` correctly refuses `validator:*` to all but the validator/register principals (#1397/#1424). US4 made the SSR **proposable but not approvable**. Evidence on #1400.
- [X] T063 [US4] 🔴 **LIVE GATE** Confirm the former owner can no longer govern and the new owner can — **PASSED 2026-08-17 on n1**, both directions, on register `c3717b023df34eddba39f6c4f1f3d971`. The former owner proposing a Transfer is refused by name (`400 "Only the Owner can propose an ownership transfer"`); the new owner's `Add` proposal is accepted **and seals** (docket 4). Run against an ordinary register rather than the SSR for the T062 reasons above; the authority rule under test is register-agnostic.
...
- [ ] T069 Delete the throwaway probe registers on n1 once no longer needed as evidence — **still deliberately NOT done; re-checked 2026-08-17 (#1403).** ⚠ **The three registers this task used to name (`c794c86c…`, `96c421c0…`, `6b0760aa…`) no longer exist** — they predate the 2026-08-10 re-genesis and were wiped with it, so a future sweep must work from a live inventory rather than from this list. Two further reasons it stays held: the US4 gates did **not** pass (T062 above), so the "once the feature closes" precondition is unmet and several targets are now live evidence for open issues (`a17208cf…` backs PR #1463's `Unanimous`→`StrictMajority` finding; `c3717b023df3…` and `e89a93f7b803…` back #1400/#1464); and **the assumed mechanism does not work** — `DELETE /api/registers/{id}` matches the caller's `wallet_address` claim against the genesis Owner attestation, and a platform-tier admin token carries no such claim at all, so it returns **403** for every register (probed live). Resolving that — or deciding the endpoint should accept a platform SystemAdmin — is a prerequisite for the sweep. Current inventory on #1403. The original note follows: **deliberately NOT done.** The task is conditional on the registers no longer being evidence, and they still are: they back gates recorded as passed, and the newer `a17208cf…` / `6294841e…` back T048/T049 including the cross-node comparison above. `DELETE /api/registers/{id}` exists and is owner-attested, so this is doable — but it is irreversible on a live node carrying the AIAS demo, and destroying the evidence for gates while the feature is still open is the wrong order. Do it as a deliberate sweep once US4 closes, and include the T048/T049/T057 registers in the same pass.
...
- [X] T095 Enforce **Owner-only** delegation granting (R-023) — **resolved by failing closed, because the task's premise does not hold.** There is no granting path to enforce Owner-only *in*: `GovernanceDelegation` is carried by a submission and verified by the validator, and issued by nothing.

> **But "no granting path" is not "unreachable", and that gap was the actual hole.** A hand-crafted submission needs no UI. Every cryptographic check on the delegated path is sound — the grant must be signed by a key that genuinely belongs to the individual named as granting it (FR-035), it empowers exactly one approver key, and scope, expiry and revocation are all enforced. What nothing checks is whether that individual had any **authority** to grant: no roster check, no Owner check, not even organisational affiliation. So anyone able to produce the organisation's signature plus a wallet key of their own could name themselves as the empowering individual and have a machine approve for them.
>
> That does **not** escalate authority — the organisation's signature is still required, so it is bounded by the R-006 custody limitation (T068/#1380). What it degrades is the accountability record, from "a named individual consented" to "somebody asserted they were entitled to delegate", which is the one thing FR-029 exists to prevent.
>
> **Implemented:** `AuthorisationRefusalReason.DelegationNotAvailable`, returned before any delegation check so nothing about a particular delegation can change the answer, in the shared `GovernanceAuthorisationValidator` so intake and the per-node recount refuse identically (the T079 lesson). Mapped to **501 Not Implemented**, not the 422 default — the submission may be perfectly well-formed, and calling it unprocessable sends an integrator hunting a fault that isn't there.
>
> **The verification code is kept, not deleted.** It is correct and covered, and a real granting path needs exactly it. The gate is a defaulted `allowDelegated` parameter rather than a constant so that coverage keeps running and opts in explicitly; production reaches the validator through one call site that never passes it, so the refusal is unconditional in practice and any caller passing `true` is a greppable bug. Lift the refusal in the same change that adds granting.
>
> **The grant path itself is a feature, not a tail task** — filed separately. Until it exists, R-017's "an autonomous approver is delegated, not unaccountable" remains a type system with no way to enter it.
...
- [X] T096 Interactive signing windows to 15 minutes, scripted flows unchanged at 5 (R-023) — **closed as not applicable: the premise is factually wrong.** Nothing expires mid-review, so there is no window to extend. The 5 minutes is `RegisterCreationOrchestrator._pendingExpirationTime`, which bounds register **creation** (the scripted two-phase initiate/finalize ceremony R-023 explicitly says to leave alone). An approval signing request carries no window of its own — `GovernanceSigningRequest.ExpiresAt` is assigned `operation.ExpiresAt`, i.e. the **proposal's** expiry, which is 7 days by default (`PolicyGovernanceConfig.ProposalTtlDays`). Verified in source, not inferred from the task note. **Not a blocker for T083**, which was the only reason it was sequenced early. Revisit only if a future signing surface introduces a short-lived session of its own — and then it is that surface's task, not this one.
