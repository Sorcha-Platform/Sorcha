...
2. **Given** the citizen has submitted the licence application, **When** Acme Licensing Co. requests identity verification, **Then** the citizen is prompted to present the specific claims that were requested (given name, family name, date of birth, portrait), with the request clearly listing each claim.
3. **Given** the citizen consents to the presentation, **When** the platform presents the credential, **Then** only the requested claims are disclosed — the email and address claims are withheld from Acme Licensing Co.
4. **Given** the licensing officer opens the pending licence application, **When** the review screen renders, **Then** it shows two stacked credential cards: the presented identity on top (with disclosed claims populated and withheld claims shown as faded placeholders with an explanatory label) and the licence-to-be below (in a distinct colour theme signalling the different credential type, with a "pending" watermark).
5. **Given** the licensing officer approves, **When** the licence credential is minted, **Then** the credential carries the licence number, vehicle class, issue date, expiry date, holder name, holder date of birth, and holder portrait (if the citizen chose to present a portrait).
...
8. **Given** the citizen did not provide a portrait for their Assured Identity, **When** the licence is issued, **Then** the licence is still issued and the portrait claim on the licence is also absent.
...
- **FR-028**: The actor definitions for the walkthrough MUST include rules-mode configurations for the verification-analyst and licensing-officer roles that stamp approve on valid applications.
...
- **Verification Analyst** — A staff role within the Assured Identity issuing organisation (Acme Verification Co.) who reviews citizen applications and approves credential issuance. May be filled by a human user or a background agent in rules or AI mode.
- **Licensing Officer** — A staff role within Acme Licensing Co. Verifies the citizen's presented identity and approves driving licence issuance. Same agent/human duality as the verification analyst.
...
- **Driving Licence Credential** — A Verifiable Credential issued by Acme Licensing Co. asserting the citizen's right to drive a specified vehicle class until a specified expiry date. Carries the citizen's portrait when the Assured Identity's portrait was presented.
...
This feature is delivered in **seven sequential phases**, grouped into three shippable units. Each phase delivers a discrete, reviewable increment. Phases 1–2 ship the platform polish (Cluster A + identity blueprint); phases 3–4 ship the chain-consumption proof (licensing + cross-peer); phases 5–7 consolidate and clean up.
