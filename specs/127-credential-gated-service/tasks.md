...
- [ ] T028 [P] [US1] Add new optional method `Task<ConsumerInitiationDescriptor> BuildInitiationAsync(PresentationInitiationContext, CancellationToken)` to `src/Common/Sorcha.PresentationLifecycle.Abstractions/IPresentationConsumer.cs` as a default-throws default interface method per `contracts/presentation-consumer-buildinitiation.md`. HAIP's existing `IPresentationConsumer` impls continue to compile unchanged.
- [ ] T029 [P] [US1] Add `ConsumerInitiationDescriptor` record `(string AuthorizationRequestUri, string? RequestUri, string? Nonce)` to `src/Common/Sorcha.PresentationLifecycle.Abstractions/` next to `IPresentationConsumer.cs`.
- [ ] T030 [P] [US1] Create `src/Services/Sorcha.Blueprint.Service/Services/Implementation/SorchaWalletPresentationConsumer.cs` — implements `IPresentationConsumer` with `ConsumerName = "sorcha-wallet"`. `VerifyAsync` deserialises the wallet's signed VP from the F111 callback payload, invokes `Sorcha.Verifier.Engine`, returns `PresentationOutcome.Success` with verified claims filtered to `context.RequiredClaims`, or `PresentationOutcome.Decline` with reason code (`expired-credential` / `revoked` / `wrong-issuer` / `signature-invalid` / `claims-missing`). `BuildInitiationAsync` returns an OID4VP `openid4vp://` URI carrying the council DID, presentation_definition derived from `context.CredentialRequirement`, nonce, and `response_uri` resolving to F111's existing `/api/presentations/callbacks/sorcha-wallet/{requestId}` endpoint.
- [ ] T031 [US1] Create `src/Services/Sorcha.Blueprint.Service/Storage/Presentations/IClaimsFetchTokenStore.cs` + `RedisClaimsFetchTokenStore.cs`. `StoreAsync(token, presentationRequestId, ttl)` writes via `SET NX`; `GetAndRemoveAsync(token)` returns the bound `presentationRequestId` and atomically deletes (NonceStore pattern; first-fetch wins). Wire via `IAtomicDistributedCache` for consistency with F126's nonce store pattern.
- [ ] T032 [US1] Extend `src/Services/Sorcha.Blueprint.Service/Services/Implementation/PresentationLifecycleService.cs.InitiateAsync` — after writing `presentation-initiated`, dispatch to the resolved consumer's `BuildInitiationAsync` (falling back to the existing hardcoded HAIP path on `NotSupportedException`). For consumers that opt into claims-fetch (Sorcha-wallet today; identified via a new `ConsumerSupportsClaimsFetch` predicate or via overriding a `ProducesDisclosedClaimsForCouncilPage` flag on `IPresentationConsumer`), mint a `ClaimsFetchToken` via `IClaimsFetchTokenStore.StoreAsync` and include it in the returned `PresentationInitiationResult`.
- [ ] T033 [US1] Extend `PresentationInitiationResult` (in `src/Services/Sorcha.Blueprint.Service/Services/Interfaces/IPresentationLifecycleService.cs`) with an optional `string? ClaimsFetchToken` field. Existing HAIP callers receive `null`; the new Sorcha-wallet path returns a fresh single-use value per F127's claims-fetch contract.
...
- [ ] T035 [P] [US1] Extend `src/Services/Sorcha.Blueprint.Service/Hubs/BlueprintHubGroups.cs` — add `public static string PresentationNonce(Guid presentationRequestId) => $"presentation:{presentationRequestId:N}";`. CI grep gate (Feature 118 `check-no-inline-group-strings.ps1`) covers this since it enforces builder usage globally.
- [ ] T036 [P] [US1] Extend `src/Services/Sorcha.Blueprint.Service/Hubs/IBlueprintHubClient.cs` — add `Task PresentationOutcomeReady(string presentationRequestId)` with XML doc per Feature 118 thin-signal convention. The `<see cref="…"/>` points at the disclosed-claims endpoint as the council page's next call.
- [ ] T037 [US1] Wire `PresentationLifecycleService.HandleOutcomeAsync` to publish `IBlueprintHubClient.PresentationOutcomeReady(requestId)` to `BlueprintHubGroups.PresentationNonce(requestId)` immediately after the `presentation-outcome` tx is written, for both `success` and `decline` kinds. (Council page polls the F111 status endpoint to learn the kind.)
- [ ] T038 [US1] DI wiring in `src/Services/Sorcha.Blueprint.Service/Program.cs`: register `SorchaWalletPresentationConsumer` as `IPresentationConsumer` (alongside the existing `HaipPresentationConsumer`); register `IClaimsFetchTokenStore` → `RedisClaimsFetchTokenStore` with the storage registration log (`storageLog.RegisterPersistent(...)` per Feature 113); ensure `Sorcha.AtomicCache` ProjectReference is added to `Sorcha.Blueprint.Service.csproj` if not already present (T007 deferral from PR-A).
