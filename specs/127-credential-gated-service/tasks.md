...
- [x] T034 [US1] Extend `src/Services/Sorcha.Blueprint.Service/Endpoints/PresentationEndpoints.cs` with `GET /api/presentations/{presentationRequestId:guid}/disclosed-claims` per `contracts/disclosed-claims-endpoint.md`. Implements: validate `?token=` via `IClaimsFetchTokenStore.GetAndRemoveAsync`; assert bound `presentationRequestId` matches the path param; read disclosed claims from the new `IDisclosedClaimsStore` (Redis stash written alongside the outcome tx — avoids re-decrypting the register record); return `DisclosedClaimsResponse` (success / pending / 410 decline / 410 abandoned / 410 claims-expired / 401 token-invalid / 401 token-mismatch). `AllowAnonymous`, OpenAPI documented. Token consumed on terminal-state fetches; pending-state fetches also consume so the council page must reuse the same token on retry (council page subscribes to the SignalR signal so pending-state fetches should be rare).
...
- [x] T039 [P] [US1] `tests/Sorcha.Blueprint.Service.Tests/Services/SorchaWalletPresentationConsumerTests.cs` — 11 cases covering: consumer-name; Success path; minimal-disclosure claim filtering; SchemaMismatch on missing required claims; theory-driven decline-reason mapping (revoked / expired / wrong-issuer / signature-invalid / claim mismatch / catch-all); VerifierError on missing session; VerifierError on unexpected payload type; JsonElement payload deserialisation; VerifierError when validator throws; OID4VP URI shape verification; nonce freshness per call. All pass.
- [x] T040 [P] [US1] **Partial** — `tests/Sorcha.Blueprint.Service.Tests/Storage/Presentations/RedisClaimsFetchTokenStoreTests.cs` (6 cases) + `RedisDisclosedClaimsStoreTests.cs` (5 cases). Covers input validation, GetAndRemove on hit / miss / unparseable / empty, and JSON round-trip on the disclosed-claims store. The two arg-capture assertions on `StringSetAsync` write shape were dropped — Moq's overload matching against `StackExchange.Redis` instance+extension dispatch is fragile; better covered by an integration test with a real Redis (full T040 unblock with Testcontainers alongside T041-T043).
...
- [x] T044 [P] [US1] Created new `BlueprintHubConnection` in `src/Apps/Sorcha.UI/Sorcha.UI.Components.User/Services/Shared/` (mirrors `TenantHubConnection`). Exposes `OnPresentationOutcomeReady` event. Accepts optional `accessTokenProvider` so the council-page case (unauthenticated, subscription keyed by high-entropy nonce) works without an auth context. Tracks subscribed groups locally so automatic reconnects re-join.
- [x] T045 [P] [US1] Created `IPresentationSignal` with event-based shape (mirrors F126's `IEnrolPairingSignal`): `event Func<PresentationSignalOutcome, Task>? OnOutcomeReady`, `event Action? OnFallbackEngaged`, `event Action? OnManualRecoveryRequired`. `PresentationSignalOutcome` carries `(Guid PresentationRequestId, string Kind)` where Kind is the F111 lifecycle terminal state.
- [x] T046 [US1] Created `PresentationSignal` (mirrors `EnrolPairingSignal`). 2 s hub-connect window, 3 s polling cadence on F111's `/status` endpoint, 60 s manual-recovery ceiling. Uses `TimeProvider` for testability (passed into `Task.Delay`). The hub signal triggers a status fetch (since the hub-event is thin); polling also reads the same `/status` endpoint. Either path raises `OnOutcomeReady` once with the terminal kind; subsequent transports become no-ops.
- [x] T047 [US1] Created `CredentialGateComponent.razor` with a slightly refined contract: takes a `CredentialGateInit` parameter (PresentationRequestId, AuthorizationRequestUri, ClaimsFetchToken) the consumer page hands in AFTER it submits the starting action. The component owns the QR + signal + claims-fetch UX; the page owns the action-submission HTTP call (so the page can carry its own auth, retry policy, error UX). Renders states: Waiting (QR shown), Succeeded (ChildContent), Declined / Abandoned / ManualRecoveryRequired / NoCredentialAvailable (error variants). Disposable; cancels on unmount.
- [x] T048 [US1] DI registration in `samples/strathcarron-portal/Program.cs` — calls the new `services.AddSorchaPresentationGate(builder.HostEnvironment.BaseAddress)` extension from the library, which registers `BlueprintHubConnection` (scoped) + `IPresentationSignal` (typed HTTP client). Sample `_Imports.razor` extended with the `CredentialGate` / `Models.User.Presentation` / `Services.User.Presentation` namespaces so pages can reference them without explicit `@using`.
- [x] T049 [P] [US1] `tests/Sorcha.UI.Core.Tests/Components/CredentialGate/CredentialGateComponentTests.cs` (bunit) — 8 cases: NoGate renders ChildContent immediately; Waiting state renders the QR + prompt copy; Success outcome fetches claims + renders ChildContent + fires OnPresented; Decline outcome renders decline message; Abandoned outcome renders timeout message; ManualRecoveryRequired event renders the recovery message; Success outcome without ClaimsFetchToken degrades to Decline (no HTTP fetch fires); claims-fetch returning 410 maps to the abandoned UX. All pass.
- [x] T050 [P] [US1] `tests/Sorcha.UI.Core.Tests/Services/User/Presentation/PresentationSignalTests.cs` — `FakeTimeProvider`-driven, 6 cases: rejects empty requestId; polling fires OnOutcomeReady when status reaches terminal; polling ignores pending state and keeps looping; manual-recovery fires after 60 s with no signal; manual-recovery suppressed after a terminal outcome; fallback-engaged fires when hub fails fast. Fallback-engaged test caught an implementation gap (engageReason was only set on timeout, not on hub-connect-failed); fixed in `PresentationSignal.StartAsync`. All pass.

### Blue Badge content (PR-C)

- [x] T051 [P] [US1] `walkthroughs/Strathcarron/blueprints/strathcarron-blue-badge.json` — three-action chain. `verify-identity` (starting, citizen, `credentialRequirements[].presentationSource: "SorchaWallet"`, `type: "AssuredIdentityCredential"`, requiredClaims for givenName/familyName/dateOfBirth/homeAddress, revocationCheckPolicy FailClosed); `submit-blue-badge` (citizen, schema with mobilityCondition + previousBadgeNumber, `x-persona.presentation: "verify-identity"`); `issue-blue-badge` (licensing-officer, credentialIssuance of BlueBadgeCredential to SorchaLocalWallet). instanceReference with BB prefix. routes between actions.
- [x] T052 [P] [US1] `samples/strathcarron-portal/Components/BlueBadgeForm.razor` — MudPaper-shelled form. "Verified ✓" alert + identity chips rendered from disclosed claims; mobilityCondition (required, helper copy) and previousBadgeNumber (optional) input fields; submit button bound to `EventCallback<BlueBadgeSubmission> OnSubmit`. data-testids `blue-badge-mobility`, `blue-badge-prev`, `blue-badge-submit` per spec. JsonElement / object claim values rendered tolerantly.
- [x] T053 [US1] `samples/strathcarron-portal/Pages/BlueBadge.razor` — `@page "/services/blue-badge"`, `@layout CouncilLayout`. Composition: `EnrolGateComponent` → `CredentialGateComponent` (LinkBackUrl="/services/driving-licence", NameOfMissingCredentialType="Assured Identity") → `BlueBadgeForm`. Page owns the F111 initiation HTTP call: posts to `/api/blueprint/blueprints/{id}/instances` with `startingActionId="verify-identity"`, receives (PresentationRequestId, AuthorizationRequestUri, ClaimsFetchToken), hands a CredentialGateInit to the gate. On form submit, posts to `/api/blueprint/presentations/{id}/submit-action` with the Blue-Badge-specific payload. Resolve-blueprint-id is a PR-C placeholder pending PR-D config/state.json wire-up; surfaces a graceful operator-facing notice when no blueprint id is available.
- [x] T054 [US1] `samples/strathcarron-portal/Pages/Index.razor` — Blue Badge card switched from "coming soon" placeholder to live `<a href="/services/blue-badge">`.

### Walkthrough seed (PR-C)

- [x] T055 [US1] `walkthroughs/Strathcarron/setup-blue-badge-demo.ps1` — reads Spec 3 state.json (fails fast if absent), reuses the council session, publishes the Blue Badge blueprint against the existing register, persists `blueBadgeBlueprintId` + `blueBadgePage` back into state.json. Idempotent (skips if already provisioned and `-Force` isn't set). Does NOT hard-fail when the Tier-1 citizen lacks an AssuredIdentityCredential — the council page handles that via the US3 routed-back error state, which the operator can exercise in Walk 2.
- [ ] T056 [US1] **Deferred**: automating the Tier-1 device-pairing + AssuredIdentityCredential issuance from PowerShell requires PWA-side IJSRuntime access (WebAuthn passkey + sign). Same constraint that left this as an operator step at the end of F126. The new Blue Badge setup script's header documents the prerequisite walk explicitly; T056 is best landed alongside a Selenium / Playwright walkthrough harness, which is its own feature-class effort.
...
- [x] T070 [P] Two F127 instruments added to the existing `PresentationLifecycleMetrics` (meter `Sorcha.Blueprint.Service.Presentation`): `sorcha_presentation_outcome_signal_latency_seconds` (histogram, tagged by `kind`, server-side measurement from outcome-tx write → hub publish — primary SC-004 verification) and `sorcha_presentation_disclosed_claims_fetch_total` (counter, tagged by `outcome` ∈ success/pending/decline/abandoned/claims-expired/token-invalid/token-mismatch/token-missing). Wired into `PresentationLifecycleService.HandleOutcomeAsync` and the disclosed-claims endpoint handler. 721/721 tests still pass.
- [x] T071 [P] Structured-logging audit on the F127 platform-side code (`SorchaWalletPresentationConsumer`, `RedisClaimsFetchTokenStore`, `RedisDisclosedClaimsStore`, `PresentationLifecycleService` extensions, `PresentationEndpoints` claims-fetch handler) AND library-side code (`PresentationHubConnection`, `PresentationSignal`, `CredentialGateComponent`). Zero `$"..."` interpolated strings in log calls — all use structured templates with named placeholders. Constitution §VIII clean.
...
- [x] T073 [P] `.claude/skills/sorcha-architecture/SKILL.md` — new "Credential gates (Feature 127)" section: three-action chain idiom, server-surface table (extending F111), IPresentationConsumer extension, hub event, library component, cross-device coordination, storage stores, non-obvious patterns. References the §14 reconciliation in the design doc instead of restating it.
- [x] T074 [P] `docs/reference/API-DOCUMENTATION.md` — Blueprint Service "Timebound Presentation Lifecycle" section documents the F127 disclosed-claims endpoint with all response variants. The other two paths the council page touches (action submission, callback) reuse F111's existing surface.
- [x] T075 [P] `docs/reference/development-status.md` — "2026-05-16" entry under Recent Completions covers F127 PR-A/B/C/D, the F111 reconciliation, and the boundary contract.
- [x] T076 [P] `CLAUDE.md` "Feature API References" section updated — F126 council-page enrolment gate AND F127 credential gate appended to the list of features whose endpoint surface lives in the sorcha-architecture skill.
- [x] T077 [P] `.specify/MASTER-TASKS.md` — version bumped to 7.14, since-last-update note rewritten to capture the Strathcarron citizen-arc progress (Specs 2/3 shipped, Spec 4 in flight via four sub-PRs), the boundary contract, and the F127 ↔ F111 reconciliation.
- [x] T078 [P] `specs/127-credential-gated-service/checklists/requirements.md` — post-implementation revalidation section (2026-05-16, PR-D wave 2) appended confirming all original quality items still pass after the F111 reconciliation + FR renumbering.
