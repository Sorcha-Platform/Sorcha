...
- [X] T029 [US4] ✅ DONE in US5 — removed `[McpServerToolType]` from `WalletSignTool` so the assembly scan no longer discovers/registers it; it is now fully absent from the served surface (and the advertised catalogue). The class + its `[McpServerTool]` method are kept intact (compile + unit-tested) with a comment that it returns in a dedicated security-reviewed wave; restoring the type attribute re-enables it. `WalletSignToolTests` instantiate the type directly, so they still compile/pass. File: `Tools/Participant/WalletSignTool.cs`
...
- [X] T038 [US5] ✅ Extended `IToolAuditService`/`ToolAuditService` with `RecordOutcome(ToolOutcomeRecord)` recording `{ callerTier, toolName, outcome, backendStatus }` per invocation (privacy-safe structured log — no token material/arguments/subject) and emitting the metrics. Wired centrally via a new `AddCallToolFilter` (`WithToolInvocationAuditFilter` in `Infrastructure/TransportConfiguration.cs`) on **both** stdio + HTTP builders — no per-tool touch; the outcome is read from the tool result's `Status` field (camelCase or PascalCase), falling back to the protocol error flag. File: `Services/ToolAuditService.cs` + `Services/IToolAuditService.cs`
- [X] T039 [P] [US5] ✅ Added `McpMetrics` (`Sorcha.Mcp` meter via `IMeterFactory`): `sorcha_mcp_tool_invoked_total{tool,tier,outcome}`, `sorcha_mcp_backend_call_total{service,status}`, and `sorcha_mcp_tool_duration_ms` histogram. Registered as a singleton (`AddMetrics` + `AddSingleton`) and added to the OTel meter provider (`AddOpenTelemetry().WithMetrics(m => m.AddMeter(McpMetrics.MeterName))`); the OTLP exporter is wired only when `OTEL_EXPORTER_OTLP_ENDPOINT` is set (silent otherwise). Acceptance: `McpMetricsTests` + `ToolAuditServiceTests` assert the instruments record via a raw `MeterListener`. File: `Services/McpMetrics.cs`
- [X] T040 [US5] ✅ `server.json` pinned to `0.2.0`; its `tools` list aligned to the served 35-tool set (`sorcha_`-prefixed), plus a `streamable-http` remote (US3 HTTP transport). Gateway `appsettings.json` `McpManifest.Tools` reconciled to the EXACT discovered set — fixed prior drift (unprefixed names, `json_logic_test`→`sorcha_jsonlogic_test`) and dropped `wallet_sign` (T029).
- [X] T041 [US5] ✅ `ManifestIntegrityTests` reflects the `[McpServerTool(Name=...)]` set from the `Sorcha.McpServer` assembly and asserts the gateway `appsettings.json` catalogue **and** `server.json` match it exactly — failing on drift in **either** direction (tool added/removed without a manifest update, or a manifest entry with no tool). Plus a guard that `wallet_sign` is absent and the surface is non-empty. File: `tests/Sorcha.McpServer.Tests/ManifestIntegrityTests.cs`
- [X] T042 [P] [US5] ✅ Satisfied without a separate workflow: `ManifestIntegrityTests` lives in `Sorcha.McpServer.Tests`, which already runs under the `build-and-test` CI gate — so catalogue drift fails the build there. No discoverability-only CI job needed.

**Checkpoint**: ✅ Reached. Regression is observable (per-invocation `Sorcha.Mcp` metrics + privacy-safe audit) and catalogue drift is build-blocking (manifest-integrity test under `build-and-test`). Final served surface = **35 tools** (`wallet_sign` absent). McpServer.Tests: **486 passed, 0 skipped, 0 failed** (live integration tests included, gateway reachable). Build green, no new warnings.
...
### 2026-05-26 — US5 observability + manifest-integrity gate (T038–T042, T029) — branch `139-mcp-us5-observability`

**Done & verified (build green, no new McpServer warnings; McpServer.Tests 486 passed, 0 skipped, 0 failed across repeated runs — live integration tests included):**
- **T038/T039 — audit + metrics, wired centrally.** Rather than touching all 35 tools, a single `AddCallToolFilter` (`WithToolInvocationAuditFilter`) on both transport builders times every invocation, reads the caller tier from the ambient `ICallerContext` and the outcome from the tool result `Status` field (camel/Pascal, falling back to the protocol error flag), and calls `IToolAuditService.RecordOutcome` — which logs the privacy-safe `{ callerTier, toolName, outcome, backendStatus }` line and emits the `Sorcha.Mcp` metrics (`sorcha_mcp_tool_invoked_total{tool,tier,outcome}`, `sorcha_mcp_backend_call_total{service,status}`, `sorcha_mcp_tool_duration_ms`). Meter registered via `IMeterFactory` (the `IdentityMetrics` pattern), added to the OTel meter provider; OTLP exporter conditional on `OTEL_EXPORTER_OTLP_ENDPOINT`.
- **T029 — `wallet_sign` de-registered.** `[McpServerToolType]` removed from `WalletSignTool`; the served surface drops from 36 → **35**.
- **T040/T041/T042 — manifest can't drift.** `server.json` → `0.2.0` + aligned `tools` list + `streamable-http` remote; gateway `McpManifest.Tools` reconciled to the exact discovered set (was drifted: unprefixed names + `json_logic_test`). `ManifestIntegrityTests` reflects the assembly `[McpServerTool]` names and fails the `build-and-test` gate on any drift in the gateway catalogue OR `server.json`, either direction.

**Notable deviation / fix:** the two new metric tests share the (name-`const`) `Sorcha.Mcp` meter; a `MeterListener` filtering by meter name caught the *other* test class's measurements under xUnit's default class-parallelism (one intermittent `ContainSingle` failure). Fixed by putting both in a single `[Collection("Sorcha.Mcp metrics")]` so they serialise — stable across repeated runs. Each test still uses its own `IMeterFactory`/`ServiceProvider` and disposes its listener.

**Also done (housekeeping, separate commit):** `git rm` the two stray master artifacts (`walkthroughs/AssuredIdentity/crossnode-state.json`, `walkthroughs/COMPLIANCE-REPORT.md`) + matching `.gitignore` patterns.
