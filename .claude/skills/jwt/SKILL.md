...
**Validation = authenticate-broad / authorize-narrow.** The bearer pipeline accepts any of the installation's four tier audiences (`ValidAudiences = SorchaAudiences.All`), rejecting cross-installation tokens. The specific tier is enforced **per endpoint** by policies: `RequireConsumerAudience`, `RequirePlatformAudience`, and `RequireService` (`AddSorchaTierAudiencePolicies(SorchaAudiences)`). Use `AuthorizationPolicyExtensions.HasTierAudience(user, audiences, tier)` to test the predicate.
