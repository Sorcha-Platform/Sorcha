...
- The roster *decision* is unit-tested (`TransactionDistributionServiceTests`); P2P fan-out itself is not unit-provable and is validated via the cross-node walkthrough. **The residual US5 sweep — actually *removing* the `LocallyOwned` branch + topology-heuristic transport — is gated after this lands (T034 + T040 enforcement).**
