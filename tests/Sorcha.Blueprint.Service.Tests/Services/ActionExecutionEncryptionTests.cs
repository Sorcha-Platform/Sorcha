...
/// <summary>
/// Tests for recipient key resolution in <see cref="ActionExecutionService"/>.
/// Validates instance binding key resolution, register-published participant key
/// resolution via batch API, revoked participant atomic failure, and mixed-source
/// key resolution (bindings + register).
/// </summary>
public class ActionExecutionEncryptionTests
{
    ...
    public ActionExecutionEncryptionTests()
    {
        ...
        // Default: register returns empty batch response
        _mockRegisterClient
    ...
    #region Instance binding key resolution

    [Fact]
    public async Task ExecuteAsync_InstanceBindingKeys_ResolvedFromExternalKeysCorrectly()
    {
        // Arrange
        var service = CreateService();
        var instanceId = "test-instance";
        var actionId = 1;
        var walletAddress = "wallet-applicant";
        ...
        var instance = CreateTestInstance(instanceId, "blueprint-1", "register-1",
        ...
        var recipient = capturedGroups!.SelectMany(g => g.Recipients)
            .FirstOrDefault(r => r.WalletAddress == walletAddress);
    ...
    #region Register published participant keys via ResolvePublicKeysBatchAsync

    [Fact]
    public async Task ExecuteAsync_RegisterPublishedKeys_ResolvedViaBatchApi()
    {
        // Arrange
        var service = CreateService();
        var instanceId = "test-instance";
        ...
        var walletAddress = "wallet-participant";
        var publicKeyBytes = new byte[32];
        new Random(77).NextBytes(publicKeyBytes);
        var publicKeyBase64 = Convert.ToBase64String(publicKeyBytes);
        ...
        var request = new ActionSubmissionRequest
        {
            ...
            PayloadData = new Dictionary<string, object> { ["field1"] = "value1" }
            // No ExternalRecipientKeys — must come from register
        };
        ...
        // Register returns the wallet's public key
        _mockRegisterClient
            .Setup(x => x.ResolvePublicKeysBatchAsync(
                registerId,
    ...
    #region Revoked participant causes atomic failure

    [Fact]
    public async Task ExecuteAsync_RevokedParticipant_ThrowsInvalidOperationException()
    {
        // Arrange
        var service = CreateService();
        var instanceId = "test-instance";
        ...
        // Register returns revoked wallet
        _mockRegisterClient
            .Setup(x => x.ResolvePublicKeysBatchAsync(
                registerId,
        ...
        ex.Message.Should().Contain(revokedWallet);

        // Encryption pipeline should NOT be called when revoked participant detected
        _mockEncryptionPipeline.Verify(x => x.EncryptDisclosedPayloadsAsync(
            It.IsAny<DisclosureGroup[]>(),
            It.IsAny<CancellationToken>()),
            Times.Never);
    ...
    [Fact]
    public async Task ExecuteAsync_MixedKeysSources_BothResolvedCorrectly()
    {
        // Arrange
        var service = CreateService();
        var instanceId = "test-instance";
        ...
        // Register returns key only for registerWallet (external wallet excluded from batch)
        _mockRegisterClient
            .Setup(x => x.ResolvePublicKeysBatchAsync(
                registerId,
