...
| DRIFT-002 | CLI onto shared contracts — invitation slice | P1 | 16h | 🚧 | **Invitation slice DONE** (fixed a live defect: every `sorcha invitation` subcommand 400'd or mis-parsed — camelCase vs the server's snake_case, `expiresInHours` vs `expires_in_days`, a bare array vs the `{invitations,total_count}` envelope, `token` vs `invitation_token`). CLI-local `IInvitationServiceClient` + 4 DTOs deleted in favour of the shared `IRegisterInvitationServiceClient` the Blazor UI already uses; `ExtractOrgId` deduped from 4 copies into `AccessTokenClaims`. Guard: `RegisterInvitationWireContractTests` pins client↔server JSON property names (lives in Tenant.Service.Tests, which sees both sides). **Remaining:** 10 more Refit interfaces + 20 model files — Register policy (7 names), Validator (7), Wallet (6), Tenant (21). Ratchet gate lands with the final slice once the guarded type set is settled. |
