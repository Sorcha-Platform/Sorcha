...
| DRIFT-004 | Tenant auth/invitation contracts | P2 | 12h | 📋 | `TokenResponse` declared **6×** (Cli, Haip, Tenant, UI.Components.User + private copies in `Sorcha.Demo` and `ServiceAuthClient`); `CreateInvitationRequest` 4× across **two different concepts** (org-user invite vs register invite). Home already exists: `Sorcha.Tenant.Models`, referenced by Tenant.Service, both UI projects and Haip. Note `LoginRequest` is a *name collision*, not a wire break (CLI is an OAuth2 form-grant shape, server is JSON `Email`/`Password`) — disambiguate rather than merge. |
