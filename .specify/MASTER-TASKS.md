...
| GAP-021 | Agent starting-action kickoff — persona mode | P2 | 10h | 🟡 | Feature 110 US1–US4 shipped (PRs #371, #372, #373, #374) — `once` + `interval` triggers, JSONC-tolerant loader, coexistence + shutdown proven, ConstructionPermit persona parity. 106/106 agent tests green. Remaining: **wire PersonaHost through AuditLogger** (research.md R-007 — persona fires should land in `*.jsonl` audit stream alongside reactive decisions); T029/T043 live-run validation; T037 reactive-latency benchmark (deferred to walkthrough rather than unit suite). |
