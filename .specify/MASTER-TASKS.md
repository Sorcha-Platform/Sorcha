...
| UX-007 | Public user page load hits Administrator/SystemAdmin-only endpoint | P2 | 4h | 📋 | Observed 2026-04-14 on n1 — browser console shows `Authorization failed. RolesAuthorizationRequirement:User.IsInRole must be true for one of the following roles: (Administrator|SystemAdmin)` during a Consumer's first page load. Some page-load call in the Blazor client is reaching an admin-only endpoint that a public-org Consumer role should never touch. Identify the culprit (likely a shared layout service that fans out to admin-scoped APIs without a role guard), gate the call behind a role check, and silence the console warning. Not blocking but pollutes the log and points at a latent authorization boundary bug. <br><br>**2026-05-18 investigation:** Ruled out — `MainLayout.OnAfterRenderAsync` (token refresh, theme, locale, TenantHub + BlueprintHub subscriptions, `InboxApi.GetUnreadCountAsync` against `/api/me/inbox/unread-count` which is `RequireAuthorization()`), `Home.razor` (already gated by `_canSeeAdminDashboard` role check), `OrgSwitcher.GetMyOrganizationsAsync` against `/api/auth/me/organizations` (also `RequireAuthorization()`), `PairingNagBanner.IHasPairedDeviceProbe` against `/api/v1/me/devices/has-any` (bearer-auth only), `StatusFooter` against `/api/health` (anonymous), and `OperationNotificationListener` (SignalR-only). The culprit must be in a child component or page that hasn't been traced yet; needs **browser-console reproduction on n1** as a Consumer in the public org to capture the exact request URL + stack trace before a code fix. |
