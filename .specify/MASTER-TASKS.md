...
| DRIFT-005 | Dedupe trust adapters | P3 | 4h | 📋 | `TrustListAnchorProvider` verbatim-copied into Blueprint.Service (79 lines) and Haip.Service (76) — the diff is comments only. Same for `DidIssuerDirectory` and the `ITenantTrustAnchorProvider` impls. Keeping the *engine* free of network-bound providers is correct; the adapter belongs in `ServiceClients.Http` beside the existing `HttpTrustListProvider`. |
