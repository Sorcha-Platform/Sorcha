...
> **2026-06-01 — TODO (verify): genesis freshness window (`VAL_TIME_002`) — is the 1h limit actually enforced?** During the F145 cross-node deploy, genesis files **32h and ~3 days** old (`signedAt`) ingested-and-sealed cleanly on the current validator (local + n1 + tiny), with `GenesisIngestionService` submitting `CreatedAt = tx.Signature.SignedAt` (the real old timestamp, NOT re-stamped). Yet `ValidationEngine.cs:1596` computes `maxAge = isGenesis ? GenesisMaxAge : MaxTransactionAge` (both default 1h) and `IsGenesisTransaction`'s own comment says genesis "must be exempt from the freshness window … ingested arbitrarily long after the ceremony." **The code is contradictory and observed behaviour is lenient.** Trace whether the age check is bypassed/ineffective for the system-register bootstrap path, then either (a) make genesis genuinely exempt + delete the dead `GenesisMaxAge` config, or (b) fix the path so the 1h window is enforced and update the `network-bootstrap` skill accordingly (its note currently records the discrepancy). Until resolved, do NOT rely on the 1h window in either direction.
