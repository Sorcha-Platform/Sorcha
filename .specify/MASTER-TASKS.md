...
| DRIFT-002 | CLI onto shared contracts | P1 | 16h | 🚧 | **Invitation slice shipped** (#1241 — fixed every `sorcha invitation` subcommand). **Re-scoped after evidence:** migrating CLI clients onto the shared ones was the wrong fix — no shared client is a superset (Register 30 CLI methods vs 45 shared but 12 absent; Validator 23 vs 5), and most canonical types are service-internal so the CLI cannot reference them. The real harm is the CLI's **DTOs** having silently diverged from server truth. Built `Sorcha.Cli.ContractTests` — a test project referencing **both** the CLI and the services (a layering combination no production assembly may have, but the only vantage point that sees both ends of an HTTP contract). It auto-discovers pairs by name, maps CLI type → the type the endpoint actually `.Produces<T>()` (an entity that merely shares a name is not the wire shape), and asserts matching JSON property names. **Found 30 broken wire contracts.** Baselined as a shrink-only `KnownMismatch` ratchet that asserts each pair *still* differs, so it cannot decay into fiction. **Worst are the silent ones:** `rotate-secret` returns an EMPTY secret and looks successful; `audit query` always reports nothing. **Next:** shrink the baseline, silent failures first. |
