...
| DRIFT-003 | Shared validation error-code constants | P2 | 8h | 📋 | 101 distinct `VAL_*`/`CERT_*`/`TRUSTLIST_*`/`WARN_*` codes as bare string literals; only Validator.Service has a `ValidationErrorCodes.cs`. Blueprint.Engine + Blueprint.Service raise validator-namespaced codes as literals; 8 codes appear in 2+ files. These are quoted as an API surface in CLAUDE.md, the skills and `docs/` with no compile-time link. |
