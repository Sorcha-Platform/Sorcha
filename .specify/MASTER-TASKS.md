...
| DRIFT-006 | Config-key constants | P3 | 6h | 📋 | Three spellings for one value: `ServiceClients:TenantService:Address` (4 services, 8 sites), `ServiceClients:Tenant:BaseAddress` (2), and a `Services:TenantService:BaseAddress` fallback in `RegisterInvitationServiceClient.cs:40`. 61 distinct raw keys total. Extend the `SorchaConnections` cascade idiom (currently connection-strings-only) to service addresses. |
