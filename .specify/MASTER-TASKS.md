...
| DRIFT-002 | CLI onto shared contracts | P1 | 16h | 📋 | `Sorcha.Cli` keeps 11 hand-rolled Refit `I*ServiceClient` interfaces + 21 model files while **already referencing** `Sorcha.ServiceClients.Http` and `Sorcha.Wallet.Contracts`. 21 duplicated DTO names with Tenant.Service alone, 7 with Validator, 7 with Register, 6 with Wallet. **Contains a live defect**: CLI `CreateInvitationRequest` emits camelCase `registerId`/`targetOrgDid` + `ExpiresInHours` against a server requiring snake_case `register_id`/`target_org_did` + `expires_in_days`, so `sorcha invitation create` (`InvitationCommands.cs:86`) is a guaranteed 400. The correct contract already exists in the referenced shared library. |
